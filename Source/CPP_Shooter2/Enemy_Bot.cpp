...
AEnemy_Bot::AEnemy_Bot()
{
	...
	if (SwordAsset.Succeeded())
	{
		...
		SwordMesh->SetRelativeLocation(FVector(10.f, -40.f, 0.f));
		SwordMesh->SetRelativeRotation(FRotator(0.f, 90.f, 90.f));
		SwordMesh->SetRelativeScale3D(FVector(0.3f, 0.3f, 0.3f)); // 1.0 / 5.0 = 0.2
	}
	else
	{
...
void AEnemy_Bot::Tick(float DeltaTime)
{
	...
	if (MovementDelta < 1.f)
	{
		...
		if (!bIsPausedAfterStop)
		{
			...
			GetWorld()->GetTimerManager().SetTimer(PauseTimerHandle, this, &AEnemy_Bot::ResumeMovement, 1.5f, false);
		}
		return;
	}
	else
...
void AEnemy_Bot::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
}

void AEnemy_Bot::TakeDamage(float DamageAmount)
{
...
void AEnemy_Bot::Die()
{
	...
	if (GM)
	{
		int32 NewCoins = GM->GetCurrentCoins() + 10;
	...
	GetWorld()->GetTimerManager().SetTimer(DeathTimer, this, &AEnemy_Bot::DestroyBot, 2.5f, false);
}

void AEnemy_Bot::ResumeMovement()
{
	if (bIsDead == true)
	{
		return;
